function initORG(){
  ...
  styleCache={};
	...
	sourceOrgkaz = new ol.source.Vector({
		format: new ol.format.GeoJSON(),
		loader: function(extent, resolution, projection) {
			var url = 'https://gs.gismart.ru/geoserver2/mainMap/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=mainMap:orgokaz&maxFeatures=500&' +
			          'outputFormat=application/json&srsname=EPSG:3857&' +
			          'bbox=' + extent.join(',')+',EPSG:3857';
			var xhr = new XMLHttpRequest();
			xhr.open('GET', url);
			var onError = function() {
				sourceOrgkaz.removeLoadedExtent(extent);
			}
			xhr.onerror = onError;
			xhr.onload = function() {
				if (xhr.status == 200) {
					sourceOrgkaz.addFeatures(sourceOrgkaz.getFormat().readFeatures(xhr.responseText));
				} else {
					onError();
				}
			}
	...
	var orgkazClusterSource = new ol.source.Cluster({
		distance: 46,
		source: sourceOrgkaz
	});
